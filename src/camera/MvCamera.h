...
public:
    ...
    int GetExposureTime(double& time);               // ch:设置曝光时间 | en:Set Exposure Time
    int SetExposureTime(double time);
    int GetGain(double& gain);                       // ch:设置增益 | en:Set Gain
...
public:
    ...
    unsigned char* m_pSaveImageBuf = nullptr; //照片的数据
    unsigned int            m_nSaveImageBufSize;
    MV_FRAME_OUT_INFO_EX    m_stImageInfo; //照片信息
